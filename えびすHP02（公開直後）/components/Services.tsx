    ...
    <section id="services" className="py-32 bg-emerald-950 text-stone-100 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row items-end justify-between mb-20 gap-8">
          ...
          <button className="group flex items-center gap-3 px-8 py-4 border border-emerald-800 bg-emerald-900/50 backdrop-blur-sm rounded-full hover:bg-emerald-800 hover:border-emerald-700 transition-all text-sm font-bold text-white">
            料金表一覧を見る
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {SERVICES.map((service, index) => (
            <div 
              key={service.id} 
              className={`relative rounded-[2rem] p-8 md:p-10 transition-all duration-500 group flex flex-col overflow-hidden
              ${index === 1 
                ? 'bg-gradient-to-br from-emerald-800 to-emerald-900 border border-emerald-600 shadow-2xl shadow-emerald-900/50 lg:-mt-8 lg:mb-8 z-10' 
            ...
            >
              ...
              {index === 1 && (
                 <div className="absolute -top-4 right-8 bg-amber-400 text-amber-950 text-xs font-bold px-4 py-1.5 rounded-full shadow-lg flex items-center gap-1">
              ...
              <div className="mb-6 relative">
                <h3 className={`text-2xl font-bold font-serif mb-2 flex items-center gap-2 ${index === 1 ? 'text-white' : 'text-stone-100'}`}>
                  {service.title}
              ...
              <p className={`text-sm mb-8 min-h-[4em] leading-relaxed ${index === 1 ? 'text-emerald-100' : 'text-stone-300'}`}>
                {service.description}
              </p>
                ...
                {service.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center gap-3 text-sm group/item">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 transition-colors
                      ${index === 1 ? 'bg-emerald-700 text-emerald-300' : 'bg-white/10 text-stone-300 group-hover/item:bg-emerald-500/20 group-hover/item:text-emerald-300'}
                      ${service.id === 'svc4' ? 'bg-pink-500/20 text-pink-300 group-hover/item:bg-pink-500/40' : ''}
                    `}>
                      <Check size={14} />
                    </div>
                    <span className={index === 1 ? 'text-stone-200' : 'text-stone-300'}>{feature}</span>
                  </li>
                ))}
              </ul>
              ...
              {service.id === 'svc4' ? (
                ...
                <button className={`w-full py-4 rounded-xl font-bold transition-all duration-300 flex items-center justify-center gap-2
                  ${index === 1 
                    ...
                    : 'bg-emerald-900/50 text-white border border-emerald-700/50 hover:bg-emerald-800 hover:border-emerald-600'
                  }
                `}>
                  詳しく見る
