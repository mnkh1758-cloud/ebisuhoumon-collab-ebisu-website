...
const staffMembers: StaffMember[] = [
  {
    ...
    clinicId: 'haiki',
    clinicName: 'えびす鍼灸整骨院',
    image: '/hirase.jpg', // デスクトップのhirase.jpgを表示（publicフォルダに配置してください）
        ...
        <div className="space-y-20">
          {filteredStaff.length > 0 ? (
            filteredStaff.map((staff) => (
              <div key={staff.id} className="max-w-5xl mx-auto bg-white rounded-[2.5rem] shadow-xl overflow-hidden border border-stone-100">
                <div className="flex flex-col md:flex-row">
                  
                  {/* Image Section */}
                  <div className="md:w-2/5 relative min-h-[400px] md:min-h-full bg-stone-200">
                    <img 
                      src={staff.image} 
                    ...
                    <div className="absolute bottom-6 left-6 text-white md:hidden">
                      ...
                      <p className="text-2xl font-serif font-bold">{staff.name}</p>
                    </div>
                  </div>
                  ...
                  <div className="md:w-3/5 p-8 md:p-12 lg:p-16">
                    <div className="hidden md:block mb-8">
                      <div className="flex items-center gap-3 mb-2">
                         <span className="bg-emerald-100 text-emerald-800 text-xs font-bold px-3 py-1 rounded-full tracking-wider">{staff.role}</span>
                         <span className="text-stone-400 text-xs font-bold tracking-widest uppercase">{staff.englishRole}</span>
                      </div>
                      <p className="text-emerald-600 font-bold text-sm mb-1">{staff.qualification}</p>
                      <h2 className="text-3xl lg:text-4xl font-serif font-bold text-stone-800 mb-1">{staff.name}</h2>
                      <p className="text-stone-400 font-serif italic">{staff.englishName}</p>
                    </div>

                    <div className="space-y-8">
                      {/* Philosophy */}
                      <div>
                        <h3 className="flex items-center gap-2 text-emerald-700 font-bold mb-3 text-lg">
                          <Heart size={20} className="fill-emerald-100" />
                          施術で心がけていること
                        </h3>
                        <div className="bg-stone-50 p-6 rounded-2xl border border-stone-100">
                          {staff.philosophyTitle && <p className="font-bold text-stone-800 mb-2 text-lg">{staff.philosophyTitle}</p>}
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        <div className="flex flex-col gap-2">
                          <span className="flex items-center gap-2 text-stone-400 text-xs font-bold uppercase tracking-wider">
                            <Clock size={14} /> 施術歴
                          </span>
                          <p className="text-xl font-bold text-stone-800 font-serif">{staff.experience}</p>
                        </div>

                        <div className="flex flex-col gap-2">
                          <span className="flex items-center gap-2 text-stone-400 text-xs font-bold uppercase tracking-wider">
                            <Award size={14} /> 得意な技術・デザイン
                          </span>
                          <p className="text-lg font-bold text-stone-800">{staff.specialty}</p>
                        </div>

                        <div className="flex flex-col gap-2 sm:col-span-2">
                          <span className="flex items-center gap-2 text-stone-400 text-xs font-bold uppercase tracking-wider">
                            <Smile size={14} /> 趣味・マイブーム
                          </span>
                          <div className="flex flex-wrap gap-2">
                            {staff.hobbies.map((hobby) => (
                              <span key={hobby} className="bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full text-sm font-bold">
                                {hobby}
                              </span>
                            ))}
                          </div>
