...
import { Menu, X, Phone, ChevronDown, CalendarClock, Briefcase, MapPin, ChevronRight } from 'lucide-react';
...
export const Header: React.FC = () => {
  ...
  const navLinks = [
    { name: 'ホーム', href: '#' },
    { name: '症状一覧', href: '#symptoms' },
    { name: '施術メニュー', href: '#services' },
    { name: '交通事故治療', href: '/traffic-accident' },
    { name: '訪問鍼灸', href: '/home-visit' },
  ...
  // Determine if we are on a page with a light background at the top
    ...
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        ...
        >
          ...
          <div className={`flex flex-col transition-colors duration-300 ${isScrolled || isLightPage ? 'text-stone-800' : 'text-stone-800 lg:text-white'}`}>
              ...
              <li key={link.name}>
                <a
                  ...
                  className={`text-sm font-medium tracking-wide transition-all duration-300 relative group py-2 ${
                    isScrolled || isLightPage ? 'text-stone-600 hover:text-emerald-700' : 'text-white/90 hover:text-white'
                  }`}
                >
                  {link.name}
                  <span className={`absolute bottom-0 left-0 w-full h-px bg-current transform scale-x-0 transition-transform duration-300 group-hover:scale-x-100 origin-left ${
                    isScrolled || isLightPage ? 'bg-emerald-600' : 'bg-white'
                  }`}></span>
                </a>
              </li>
            ))}
          ...
          <div className="flex flex-col h-full pt-24 px-6 pb-10 overflow-y-auto">
            ...
            <div className="mt-auto space-y-4">
               <Link 
                  ...
                  className="flex items-center justify-center gap-2 w-full bg-orange-500 text-white py-4 rounded-xl font-bold mb-4 shadow-md"
               >
